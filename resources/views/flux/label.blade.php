...
<ui-label {{ $attributes->class($classes) }} data-flux-label>
    ...
    <?php if (is_string($badge)): ?>
        <span class="ms-1.5 text-zinc-800/70 text-sm/5 sm:text-xs/5 bg-zinc-800/5 px-1.5 py-1 -my-1 rounded-[4px] dark:bg-white/10 dark:text-zinc-300" aria-hidden="true">
            {{ $badge }}
        </span>
    <?php elseif ($badge): ?>
